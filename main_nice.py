...
def main():
    ...
    if need_main_loop:
        ...
        for ts in u.trajectory:
            ...
            if need_qualified:
                ...
                qualified_clusters_mols = []

            current_max_size = 0
            if any([args.sz, args.dc, args.fe]):
                current_max_size = max([len(mols) for mols in all_clusters_mols]) if all_clusters_mols else 0
                if args.sz: max_sizes.append(current_max_size)
                if args.dc: dc_data.append(current_max_size / num_mols if num_mols > 0 else 0.0)

            if args.liquidity or args.density:
            ...
            if len(times) % 100 == 0:
                log_msg = f" Time: {ts.time:10.1f} ps"
                if args.nb: log_msg += f" | Valid Clusters: {len(qualified_clusters_mols):3d}"
                if args.sz: log_msg += f" | Max Size: {current_max_size:3d}"
                print(log_msg)
    ...
    if hasattr(args, 'pdf') and args.pdf and pdf_analyzer:
        ...
        try:
            ...
            if calc_mode == 'contact':
                print("\n[i] NOTE FOR CONTACT NUMBER CALCULATION:")
                print("    - Group A MUST be your target molecule/group (e.g., Peptide, Trp sidechain).")
                print("    - Group B MUST be the environment or interacting partner (e.g., Water).")
                print("    - The algorithm calculates the Hydration Density (Interactions of Group B around EACH BEAD of Group A).\n")

            sel_a_str, sel_b_str, sel_a_desc, sel_b_desc = pdf_analyzer.interactive_group_selection()

            x_range_val = None
            if calc_mode == 'distance':
                print("\n[i] IMPORTANT NOTE: The PDF value (Y-axis) may change depending on the search radius.")
                print("[i] The current algorithm is scanning distances up to 6.0 nm to obtain the full picture.")
                print("[i] Below are the plot window parameters, which are only used to crop the image saved as a PNG.")
                while True:
                    try:
                        min_r_input = input("[-] Enter the Min value of the X-axis for the plot window [Press Enter for default 0.4 nm]: ").strip()
                        min_r = float(min_r_input) if min_r_input else 0.4
                        
                        max_r_input = input("[-] Enter the Max value of the X-axis for the plot window [Press Enter for default 2.0 nm]: ").strip()
                        max_r = float(max_r_input) if max_r_input else 2.0
                        
                        if max_r <= min_r:
                            print("    Max value must be greater than Min value!")
                            continue
                        x_range_val = [min_r, max_r]
                        break
                    except ValueError:
            ...
            print("\n[i] Recommendation: It is recommended to use Bandwidth (BW) = 0.05 for the Distance plot to clearly separate the interaction peaks.")
            try:
                bw_input = input("[-] Enter the Bandwidth for the plot smoothing [Press Enter to use default 0.05 for Distance / 0.5 for Contact]: ").strip()
                if bw_input:
                    bw_val = float(bw_input)
                else:
                    bw_val = 0.05 if calc_mode == 'distance' else 0.5
            except ValueError:
                print("    Input error, automatically using default Bandwidth.")
                bw_val = 0.05 if calc_mode == 'distance' else 0.5
            ...
            if metric_data is not None:
                ...
                print(f"\n[?] Default title will be: '{title_desc}'")
                custom_title = input(f"[-] Enter a custom title for the PDF plot [Press Enter to keep default]: ").strip()
                if custom_title:
                    title_desc = custom_title
    ...
    # =========================================================
    # 12. CALCULATE FREE ENERGY SURFACE (FES) (MULTIPROCESSING)
    # =========================================================
    if hasattr(args, 'fes') and args.fes and fes_analyzer:
        print(f"\n[+] Preparing for FES (Free Energy Surface) analysis...")
        try:
            max_threads = multiprocessing.cpu_count()
            default_threads = 4 if max_threads >= 4 else max_threads
            
            print("\n========================================================")
            print("         CPU THREAD CONFIGURATION FOR FES               ")
            print("========================================================")
            print(f"[+] System detected {max_threads} available CPU threads.")
            thread_input = input(f"[-] Enter the number of CPU threads to use for FES calculation (1-{max_threads}) [Press Enter for Default: {default_threads}]: ").strip()
            
            if thread_input.isdigit():
                n_threads = int(thread_input)
                if n_threads < 1: n_threads = 1
                if n_threads > max_threads: n_threads = max_threads
            else:
                n_threads = default_threads
                
            print(f"[✔] Successfully set to use {n_threads} CPU threads for the FES core.")
            print("========================================================\n")
            ...
            print(f"\n[?] Default title will be: '{title_desc}'")
            custom_title = input(f"[-] Enter a custom title for the FES plot [Press Enter to keep default]: ").strip()
            if custom_title:
                title_desc = custom_title
            
            d_data, angle_data = fes_analyzer.calculate_fes_trajectory(
                sel_a_str, sel_b_str, final_xtc, args.gro, b=args.b, e=args.e, dt=args.dt, n_threads=n_threads
            )
